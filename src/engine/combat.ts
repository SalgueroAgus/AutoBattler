  ...
  while (turns++ < maxTurns) {
    for (const slot of slots) {
      ...
      if (u1 && u1.current.hp > 0 && t2Idx !== null) {
        const t2 = a2[t2Idx]!;
        // mana/ability check (simple: if mana full and ability kind includes 'replace')
        let dealt = 0;
        if (u1.current.mana >= u1.stats.manaMax && u1.ability.kind.includes('manaReplace')) {
          dealt = Math.max(1, Math.floor(u1.ability.value - t2.stats.defMag));
          t2.current.hp = Math.max(0, t2.current.hp - dealt);
          u1.current.mana = 0;
          log.push(`P1:${slot+1} casts for ${dealt}`);
        } else {
          const pkt = calcBasicDamage(u1, t2);
      ...
      if (u2 && u2.current.hp > 0 && t1Idx !== null) {
        const t1 = a1[t1Idx]!;
        let dealt = 0;
        if (u2.current.mana >= u2.stats.manaMax && u2.ability.kind.includes('manaReplace')) {
          dealt = Math.max(1, Math.floor(u2.ability.value - t1.stats.defMag));
          t1.current.hp = Math.max(0, t1.current.hp - dealt);
          u2.current.mana = 0;
          log.push(`P2:${slot+1} casts for ${dealt}`);
        } else {
          const pkt = calcBasicDamage(u2, t1);
