...
export class UnitCard extends Phaser.GameObjects.Container {
  ...
  private variant: 'normal' | 'compactWide' | 'compactTall';

  constructor(scene: Phaser.Scene, x: number, y: number, data: UnitCardData, width = 420, height = 600, variant: 'normal' | 'compactWide' | 'compactTall' = 'normal') {
    super(scene, x, y);
    scene.add.existing(this);
    this.variant = variant;
    // Default sizes by variant if caller passed defaults
    if (variant === 'normal' && width === 420 && height === 600) { this.widthPx = 360; this.heightPx = 640; }
    else if (variant === 'compactWide' && width === 420 && height === 600) { this.widthPx = 360; this.heightPx = 200; }
    else if (variant === 'compactTall' && width === 420 && height === 600) { this.widthPx = 180; this.heightPx = 320; }
    ...
    const stripeH = this.variant === 'normal' ? 26 : (this.variant === 'compactTall' ? 22 : 18);
    const artH = this.variant === 'normal' ? 300 : (this.variant === 'compactTall' ? 120 : 0);
    const passiveH = this.variant === 'normal' ? 200 : 0;
    ...
    if (artH > 0) {
      const artY = this.variant === 'normal' ? -h/2 + stripeH + 100 + artH/2 : -h/2 + stripeH + 60 + artH/2;
      this.art = scene.add.rectangle(0, artY, w - 32, artH, 0x0b2238).setStrokeStyle(2, theme.line).setOrigin(0.5);
      const initial = (data.name?.[0] || '?').toUpperCase();
      const initSize = this.variant === 'normal' ? '120px' : '72px';
      this.artInitial = scene.add.text(0, this.art.y, initial, { color: theme.text, fontSize: initSize, fontFamily: 'Shadows Into Light, Arial' }).setOrigin(0.5);
    }
    ...
    const statsY = ((): number => {
      if (this.variant === 'normal') return h/2 - 60;
      if (this.variant === 'compactTall') return h/2 - 24;
      return h/2 - 24; // compactWide
    })();
    ...
    if (passiveH > 0) {
      this.passiveBox = scene.add.rectangle(0, h/2 - passiveH/2 - 16, w - 32, passiveH, 0x10161d).setStrokeStyle(2, theme.line).setOrigin(0.5);
